...
void tmc2130_fillShadowRegisters(TMC2130TypeDef *tmc2130)
{
	...
	if(TMC_ARRAY_SIZE(tmc2130_RegisterConstants) == 0)
		return;
	...
	for(i = 0, j = 0; i < TMC2130_REGISTER_COUNT; i++)
	{
		...
		while(j < TMC_ARRAY_SIZE(tmc2130_RegisterConstants) && (tmc2130_RegisterConstants[j].address < i))
			j++;

		// Abort when we reach the end of the constant list
		if (j == TMC_ARRAY_SIZE(tmc2130_RegisterConstants))
			break;
